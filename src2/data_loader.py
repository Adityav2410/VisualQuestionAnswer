...
def getImageFeatures(sess, vgg, images ,img ):
    ...
    img_feature = sess.run([vgg.pool4, vgg.fc7], feed_dict={images:img_reshape})
    return(img_feature)
...
def getNextBatch(sess, vgg, images, qa_data, question_vocab, answer_vocab, datapath, batchSize=32, purpose='train'):
	...
	batchFeatures_conv = np.zeros((0,14,14,512))
	batchFeatures_fc = np.zeros((0,4096))
	batch_id = []
	...
	for iter in cycle(qa_data):
		...
		batchFeatures_conv = np.concatenate((batchFeatures_conv,imgFeatures[0]),0)
		batchFeatures_fc = np.concatenate((batchFeatures_fc,imgFeatures[1]),0)
		...
		if currIndex == batchSize:
			yield np.copy(batch_quest),np.copy(batch_ans),batch_id[:],np.copy(batchFeatures_conv), np.copy(batchFeatures_fc) 
			batch_id = []
			batch_quest 		= np.zeros((0,question_length))
			batch_ans   		= np.zeros((0,answerVocabSize))
			batchFeatures_conv	= np.zeros((0,14,14,512))
